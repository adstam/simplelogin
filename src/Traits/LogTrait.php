...
private function log(
    ...
    $debugOnly  = $definition['debugonly'] ?? false;
    ...
    if (!is_string($type) || !in_array($type, $allowedTypes, true)) {
        $type = 'AccountEvent'; // 👈 Veilige default
    }
    ...
    try {
        $db->setQuery(
            $db->getQuery(true)
                ...
                ->values(implode(',', [
                    ...
                    $db->quote($created),
                    $db->quote($status),
                    $db->quote($type), // 👈 Nu altijd een veilige string
                    $loginId !== null ? (int) $loginId : 'NULL',
                ]))
        )->execute();
    } catch (\Exception $e) {
        // Log de error, maar gooi GEEN exception (voorkomt de "Array to string conversion" warning)
        Log::add(
        ...
        error_log("Simplelogin log error - status: $status, type: $type, error: " . $e->getMessage());
    }
